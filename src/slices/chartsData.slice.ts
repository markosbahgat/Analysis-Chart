...
const chartDataSlice = createSlice({
  ...
  reducers: {
    setFilters: (
      ...
      { payload }: PayloadAction<{ key: string; value: string | null }>
    ) => {
      ...
      switch (key) {
        case "country":
          localStorage.setItem(
            "filters",
            JSON.stringify({ ...state.filters, country: value })
          );
        ...
        case "camp":
          localStorage.setItem(
            "filters",
            JSON.stringify({ ...state.filters, camp: value })
          );
        ...
        case "school":
          localStorage.setItem(
            "filters",
            JSON.stringify({ ...state.filters, school: value })
          );
    ...
    appendDataSets: (
      state: ChartDataState,
      { payload }: PayloadAction<IDataSets>
    ) => {
      if (
        state.chartDataSets.find(
          (item) => item.dataSets.label === "Testing School"
        )
      ) {
      ...
      if (
        !state.chartDataSets.find(
          (item) => item.dataSets.label === payload.dataSets.label
        )
      ) {
